import { Box, Image, Text,Divider, Drawer,
  DrawerBody,
  DrawerFooter,
  DrawerHeader,
  DrawerOverlay,Input,
...
const Navbar = () => {
  ...
  <Box h="100px" alignItems={"center"}  justifyContent={"space-around"} postion="sticky" top={0} display={"flex"} boxShadow=" rgba(149, 157, 165, 0.2) 0px 8px 24px;" >
      ...
      <Box display={{base:"none",md:"flex"}} h="100%" justifyContent={"center"} alignItems="center" gap="10px">
      <SearchIcon mr="20px" fontSize={"2xl"}/>
      <Image h="40%" src={wa}></Image>
        <Divider colorScheme="blackAlpha" borderColor={"black"} h="40px" orientation='vertical'/>
      <Image h="40%" src={call}></Image>
      </Box>
      <Box display={{base:"block",md:"none"}}>
        <Box display="flex" >
      <SearchIcon mr="20px" fontSize={"2xl"}/>
          <GiHamburgerMenu fontSize={"25px"} ref={btnRef} onClick={onOpen}></GiHamburgerMenu>
        </Box>
      <Drawer
        isOpen={isOpen}
        placement='right'
      ...
      >
        ...
        <DrawerContent>
          <DrawerCloseButton />
          <DrawerHeader><Image h="30px" src="https://www.findmyacco.com/image/logo.png"></Image></DrawerHeader>

          <DrawerBody display={"flex"} flexDir="column" gap={"20px"}>
            <NavLink  color="red" to="/about">About us</NavLink>
            ...
            <AccordionItem border="none">
                <AccordionButton  p="0px" >     
                  <Box w="100%"display={"flex"} justifyContent="space-between"><Text >Student Accomodation</Text><ChevronDownIcon fontSize="25px"/></Box>
                </AccordionButton>
              <AccordionPanel>
